...
export class WysiwygEditor {
  ...
  private _elementPropsEnabled: boolean;

  constructor(opts: WysiwygOptions = {}) {
    this.opts = opts;
    this._elementPropsEnabled = opts.toolbar?.elementProps !== false;
  ...
  private setupDrawClickHandler(): void {
    this.editorEl.addEventListener('click', (e) => {
      ...
      if (target) {
        ...
        if (this._elementPropsEnabled) this.openDrawPropsModal(target);
      }
    });
  }
  ...
  private setupElementInspector(): void {
    ...
    this.editorEl.addEventListener('dblclick', (e) => {
      if (!this._elementPropsEnabled || this.sourceMode) return;
  ...
  private showElemToolbar(el: HTMLElement | null): void {
    ...
    this.hoveredEl = el;
    if (!el || !this._elementPropsEnabled) { this.elemToolbarEl.style.display = 'none'; return; }
  ...
  private buildToolbar(): HTMLElement {
    ...
    if (this.show('elementProps')) {
      const epBtn = this.makeBtn(icn.elementProps, 'Propriétés des balises (double-clic)', () => this.toggleElementProps(), 'elementProps');
      epBtn.classList.toggle('be-wysiwyg__btn--active', this._elementPropsEnabled);
      s0.push(epBtn);
    }
  ...
  private toggleElementProps(): void {
    this._elementPropsEnabled = !this._elementPropsEnabled;
    const btn = this.toolbarEl.querySelector<HTMLElement>('[data-id="elementProps"]');
    btn?.classList.toggle('be-wysiwyg__btn--active', this._elementPropsEnabled);
    if (!this._elementPropsEnabled) this.hideElemToolbar();
  }
...
const icn = {
  ...
  elementProps: S('<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/>'),
